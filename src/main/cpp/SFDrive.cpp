...
bool SFDrive::PIDDrive(float inches, float maxVel, float timeout, bool ZeroVelocityAtEnd){
   ...
   if (ZeroVelocityAtEnd)
   {
      ...
      {
         if(stopThread)
         {  frc::DriverStation::ReportError("stop threda");
            stopThread = false;
            break;
         }
         //handle timing
         ...
         frc::DriverStation::ReportError("SETPOINT: "+ std::to_string(setPoint));
      }
   }
   else
   {
...
bool SFDrive::PIDTurn(float degreesClockwise, float radius, float maxVel, float timeout, bool ZeroVelocityAtEnd){
   ...
   endPoint = (((endAngle / 360.0f) * ((radius + m_wheelTrack ) * m_PI)) / m_wheelCircumference) * m_ticksPerRev;
